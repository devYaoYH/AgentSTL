...
#include "src/heuristic/NilssonSequence.h"
#include "src/heuristic/PatternDatabaseHeuristic.h"
...
int help(){
    printf("Usage: ./npuzzle_test -p <astar|simha|pimha|all> [-n: n*n dims] [-x: dim_x] [-y: dim_y] [-w: weight] [-s scrambles] [-t times]\n");
    return 1;
}
...
int main(int argc, char* argv[]){
    ...
    while((c = getopt(argc, argv, "s:x:y:n:w:p:t:")) != -1){
        switch(c){
            case 'n':
                d = std::atoi(optarg);
                dim_x = d;
    ...
    Heuristic* nilsson = new NilssonSequence();
    std::vector<Heuristic*> hs{np_heu, nilsson};
    // Need to define pattern database heuristics before we scramble so they track distance to goal
    const int size = 100, min = 2, max = 8, num_h = 5;
    for(int i = 0; i<num_h; i++){
        Heuristic *p = new PatternDatabaseHeuristic(*np, size, min, max);
        hs.push_back(p);
    }
    ...
    if (algo.compare("all") == 0){
        ...
        Agent* simha = new SerialImhaSearchAgent(np, hs, weight);
        Agent* pimha = new ParallelImhaSearchAgent(np, hs, weight);
    ...
    for(int t = 0; t < times; t++){
        std::cout << "Running iteration " << t << std::endl;
